...
def next_batch(Minibatch_Size, Epoch_Num, Matrix_View1, Label_Vector):
    ...
    if ((End_ago + Minibatch_Size) < num_examples + 1):
        ...
        return Matrix_View1_Minibatch, Label_Vector_Minibatch
    else:
...
def save_all(View_Name, k, Address):
    ...
    for test_stage in range(1, k+1):
        a = {}
        a = loadmat(Address + '\Stage_' + str(test_stage) + '_y_Gold_y_Deep_NN_' + View_Name[0] + '.mat')
    ...
    Name = Address + '\Model_' + View_Name[0] + '.mat'
    savemat(Name, Result)
    return


def evaluation(View_Name, k, Address):
    D = loadmat(Address + '\Model_' + View_Name[0] + '.mat')
    SE = np.zeros((k,))
    SP = np.zeros((k,))
    F1 = np.zeros((k,))
    AUC = np.zeros((k,))
    ...
    Name = Address + '\Evaluation_Metrics_' + View_Name[0] + '.mat'
    savemat(Name, a)
    return
...
def model(X_Image1, update_ops_collection, is_training):
    # ***** ResNet50_Monomodal ***** :
    MaxPool = part1(X_Image1, update_ops_collection, is_training)
    Addition3_res2 = part2(MaxPool, update_ops_collection, is_training)
    Addition4_res3 = part3(Addition3_res2, update_ops_collection, is_training)
    Addition6_res4 = part4(Addition4_res3, update_ops_collection, is_training)
    Before_Softmax = part5(Addition6_res4, update_ops_collection, is_training)
    return Before_Softmax
...
def run_deep_model(View_Name, randsampled_benign_masses, randsampled_malignant_masses, splitting_benign_masses, splitting_malignant_masses, test_stage, Address):
    ...
    # ***** Model *****
    Before_Softmax = model(X_Image1, update_ops_collection, is_training)
    ...
    for Epoch_Num in range(numEpochs):
        if Epoch_Num > 1 and diff < Training_Diff:
            ...
            Train_Augmented_Matrix_View1_Minibatch, Train_Augmented_Label_Vector_Minibatch = next_batch(Minibatch_Size, Epoch_Num, Train_Augmented_Matrix_View1, Train_Augmented_Label_Vector)
            training_OP.run(feed_dict={X1: Train_Augmented_Matrix_View1_Minibatch, y_Gold: Train_Augmented_Label_Vector_Minibatch, is_training: True})
            if Epoch_Num % 10 == 0:
                Train_Cost = sess.run(Loss_fun, feed_dict={X1: Train_Augmented_Matrix_View1_Minibatch, y_Gold: Train_Augmented_Label_Vector_Minibatch, is_training: True})
    ...
    y_Deep_NN_Test = sess.run(y_Deep_NN, feed_dict={X1: Test_Matrix_View1, is_training: False})
    ...
    Name = Address + '\Stage_' + str(test_stage) + '_y_Gold_y_Deep_NN_' + View_Name[0] + '.mat'
    savemat(Name, a)
    sess.close()
    return
...
for View_Num in range(View_Names.shape[0]):
    for test_stage in range(test_stage_1, test_stage_2 + 1):
        ...
        a = {}
        a = loadmat(Address + '\Stage_' + str(test_stage) + '_y_Gold_y_Deep_NN_' + View_Names[View_Num][0] + '.mat')
